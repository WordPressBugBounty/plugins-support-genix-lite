...
ApbdWps_LoadCore('ApbdWpsQueryBuilder');

if (!class_exists("ApbdWpsModel")) {
    class ApbdWpsModel
    {
        ...
        function __construct()
        {
            ...
            $this->queryBuilder                = new ApbdWpsQueryBuilder();
            $this->tableShortForm              = "";
        ...
        {
            $args    = func_get_args();
            $args[0] = ApbdWps_Lan__($args[0], $this->app_base_name);
            if (count($args) > 1) {
        ...
        protected function CheckBasicCheck()
        {
            if (empty($this->tableName)) {
                ApbdWps_AddModelErrorsCode("E002");
        ...
        {
            $db_prefix = (('users' === $this->tableName) ? $this->db->base_prefix : $this->db->prefix);

            if (! empty($this->tableShortForm)) {
                if ($isOnlyTableName) {
                    return $this->tableShortForm;
                } else {
                    return $db_prefix . $this->tableName . " as " . $this->tableShortForm;
                }
            }
                ...
                if ($isNew || (isset($this->setProperties[$key]) && empty($this->setOption[$key]))) {
                    ...
                    if (is_array($rules)) {
                        ...
                        if ($this->IsHTMLProperty($key)) {
                            $this->{$key}(ApbdWps_KsesEmailHtml($this->$key));
                        } else {
        ...
        function Save()
        {
            ...
            if ($this->db->insert($db_prefix . $this->tableName, $insertarray) !== false) {
                ...
                ApbdWps_AddQueryError($this->db->last_query);
                if (! empty($this->db->last_error)) {
                    ApbdWps_AddQueryError($this->db->last_error);
                }
            }
        ...
        {
            $db_prefix = (('users' === $this->tableName) ? $this->db->base_prefix : $this->db->prefix);
            $query  = "SELECT max($fieldName) as lastS from " . $db_prefix . $this->tableName;
            $result = $this->db->get_row($query);
            if ($result) {
        ...
        {
            return ApbdWps_PostValue($index, $default);
        }

        function GetValue($index, $default = NULL)
        {
            return ApbdWps_GetValue($index, $default);
        }
        ...
        function SetFromArray(&$dataarray, $isNew = false)
        {
            foreach ($dataarray as $key => $value) {
                if (property_exists($this, $key)) {
                    ...
                    if ($isHtml) {
                        $NewValue = ApbdWps_KsesEmailHtml($value);
                        $oldValue = $this->PostValue("old_" . $key);
                        if (! empty($oldValue)) {
                            $oldValue = ApbdWps_KsesEmailHtml($oldValue);
                        }
                    } else {
                        $NewValue = $value;
                        $oldValue = $this->PostValue("old_" . $key);
        ...
        function Update($notLimit = false, $isShowMsg = true, $dontProcessIdWhereNotset = true)
        {
            ...
            if ($this->IsSetDataForSaveUpdate() && count($this->updateWhereExtraField) > 0) {
                ...
                $query          = $this->GetUpdateDB()->getUpdateQuery($db_prefix . $this->tableName, $notLimit);
                self::$quries[] = $query;
        ...
        function Delete($notLimit = false, $isShowMsg = true, $dontProcessIdWhereNotset = true)
        {
            ...
            if (count($this->updateWhereExtraField) > 0) {
                ...
                $query          = $this->GetUpdateDB()->getDeleteQuery($db_prefix . $this->tableName, $notLimit);
                self::$quries[] = $query;
                $uresult        = $this->db->query($query);
                if ($uresult !== false && $uresult > 0) {
                    $this->ResetSetForInsetUpdate();
        ...
        {
            $thisobj   = new static();
            $db_prefix = (('users' === $thisobj->tableName) ? $thisobj->db->base_prefix : $thisobj->db->prefix);
            $tableName = $db_prefix . $thisobj->tableName;
        ...
        static function IsGeneratedColumn($db, $table, $column)
        {
            ...
            if ($col && !empty($col->EXTRA)) {
                $extra = strtoupper($col->EXTRA);
                // MySQL uses 'GENERATED', MariaDB uses 'VIRTUAL' or 'PERSISTENT'
                return strpos($extra, 'GENERATED') !== false
                    || strpos($extra, 'VIRTUAL') !== false
                    || strpos($extra, 'PERSISTENT') !== false;
            }
